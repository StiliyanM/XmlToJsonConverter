...
{
    private readonly IFileConverter _converter = new FileConverter();
    ...
    public async Task ConvertXmlToJsonAsync_ReturnsExpectedJson_ForValidXml()
    {
        ...
        var xmlContent = Encoding.UTF8.GetBytes("<note><to>User</to><from>Service</from><body>Test</body></note>");
        var xmlFile = XmlFile.Build("valid.xml", xmlContent);
        var expectedJson = "{\"note\":{\"to\":\"User\",\"from\":\"Service\",\"body\":\"Test\"}}";

        // Act
        var jsonOutput = await converter.ConvertXmlToJsonAsync(xmlFile);

        // Assert
        var expectedJObject = JObject.Parse(expectedJson);
        var actualJObject = JObject.Parse(jsonOutput);
        Assert.True(JToken.DeepEquals(expectedJObject, actualJObject), "The converted JSON does not match the expected structure.");
    }
    ...
    public async Task ConvertXmlToJsonAsync_ThrowsException_ForInvalidXml()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<XmlException>(() => _converter.ConvertXmlToJsonAsync(xmlFile));
    }
}